...
project.delete("/:id", projectIdValidator, (req, res) => {
  projectsModel.remove(req.valProject.id).then(flag => {
    if (flag) {
      res.status(201).json({
        error: false,
...
project.put("/:id", [projectIdValidator, projectBodyValidator], (req, res) => {
  projectsModel
    .update(req.valProject.id, req.valProjectBody)
    .then(flag => {
      if (flag) {
        res.status(200).json({
          error: false,
...
function projectBodyValidator(req, res, next) {
  ...
  if (!Object.keys(req.body).length) {
    ...
    res
      .status(400)
      .json(
        "Please pass in name, description and a completed status for project"
      );
